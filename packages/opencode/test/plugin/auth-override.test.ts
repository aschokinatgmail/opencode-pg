...
describe("plugin.config-hook-error-isolation", () => {
  test("config hooks are individually error-isolated in the layer factory", async () => {
    ...
    // F3 extracted the boot-time loop into the shared fireConfigHooks helper used by both the
    // boot path and Plugin.refireConfig, so the isolation contract now lives there.
    expect(src).toContain("plugin config hook failed")

    const pattern =
      /const fireConfigHooks[\s\S]*?Effect\.forEach[\s\S]*?Effect\.tryPromise[\s\S]*?\.config\?\.\([\s\S]*?plugin config hook failed[\s\S]*?Effect\.ignore/
