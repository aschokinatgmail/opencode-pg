...
import type { PreparedQueryConfig as PgPreparedQueryConfig, PgTransactionConfig } from "drizzle-orm/pg-core/session"
import { upgradeIfNeeded } from "../../up-migrations/effect-pg"
...
import { PgEffectDatabase } from "./db"

type MigrationConfigWithInit = MigrationConfig & { init?: boolean }

type PgEffectExecuteMethod = PgExecuteMethod | "values"

// PG's upstream PreparedQueryConfig only has { execute: unknown }. The Effect wrapper needs the
// SQLite-style { run, all, get, values, execute } vocabulary so prepared queries can expose the
// run/all/get/values methods. We extend the PG base config with those keys (run/values optional
// because relational queries only specify all/get/execute).
type PreparedQueryConfig = PgPreparedQueryConfig & {
  run?: unknown
  all?: unknown
  get?: unknown
  values?: unknown
...
// PgSession.prepareQuery uses a `mode` ("arrays" | "objects" | "raw") rather than SQLite's executeMethod.
// The Effect wrapper keeps the SQLite-style executeMethod vocabulary ("run" | "all" | "get" | "values")
// internally and translates to the generic SqlClient statement API, so we mirror the SQLite wrapper 1:1.
type PgExecuteMethod = "run" | "all" | "get"
...
{
  static readonly [entityKind]: string = "PgEffectPreparedQuery"

  /** @internal */
  joinsNotNullableMap?: Record<string, boolean>
  private jitMapper?: RowsMapper<any> | RelationalRowsMapper<any>
  private cacheConfig: WithCacheConfig | undefined
  private effectExecuteMethod: PgExecuteMethod

  constructor(
    private executor: (
      params: unknown[],
      executeMethod: PgEffectExecuteMethod,
...
> {
  static readonly [entityKind]: string = "PgEffectSession"

  constructor(readonly dialect: PgDialect) {}
  ...
  ): PgEffectPreparedQuery<T, TEffectHKT, true>
...
export abstract class PgEffectTransaction<
  ...
  TRunResult,
  TRelations extends AnyRelations = EmptyRelations,
> extends PgEffectDatabase<TEffectHKT, TRunResult, TRelations> {
  static override readonly [entityKind]: string = "PgEffectTransaction"

  constructor(
    dialect: PgDialect,
    session: PgEffectSession<TEffectHKT, TRunResult, TRelations>,
...
export const migrate = Effect.fn("migrate")(function* <TEffectHKT extends QueryEffectHKTBase>(
  ...
  if (newDb) {
    // PG journal table: timestamptz for applied_at per plan decision #3; bigint identity for id.
